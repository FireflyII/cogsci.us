...
cond = "", clearCompletion && (cond += "cC"), clearProgress && (cond += "cP"), clearRules && (cond += "cR");
d = new Date();
...
var playState = {
    ...
    create: function() {
        ...
        */
        ...
        // Start a timer that will ask the player if they want to continue after 10 minutes,
        // and not allow them to end before that time...
        game.time.events.add(Phaser.Timer.MINUTE * 10, this.tenMins, this);
        tenmins = false;
        game.time.events.add(Phaser.Timer.MINUTE * 3, this.threeMins, this);
        threemins = false;
        ...
        // Instead of a progress bar, a numerical counter:
        pbar = game.add.text(player.x, player.y, pressedSum, { font: '12px Arial', fill: '#ffffff' });
        pbar.fixedToCamera = true;
        pbar.cameraOffset.x = 5;
        pbar.cameraOffset.y = 0;
        if (clearProgress) {
            pbar.visible = true;
        } else {
    ...
    // After ten minutes, ask if they want to continue
    tenMins: function() {
        tenmins=true;
        if (confirm("Thank you for trying for ten minutes\nif you'd like to exit the game click 'OK'\nbut feel free to click 'Cancel' and continue") == true) {
            playState.gameOver();
        }
        else {
            game.camera.shake(0.1,200);
        }
    },

    // After three minutes, set the flag to true to allow the player
    ...
    redWall: function(player, button) {
        ...
        if (map.hasTile(button.opensX, button.opensY, 1)) {
            if (map.getTile(button.opensX, button.opensY, 1).index == 32) {
                ...
                //if (pressedSum == 8 && cond.includes("cP") && cond.includes("cC")){
                //    playState.popDone();
                //}
            }
        }
    ...
    // In the event that the player has been told how many buttons to find, and how many they've found,
    // let them know when they're done.
    popDone: function(){
        alert("Good job, you found them all!\nPlease go to the blue button now!");
    ...
    endButton: function(player, button) {
        ...
        //player.x = 79;
        //player.y = 83;
        if (!threemins && pressedSum <8) {
            alert("There is more to do. Keep trying!");
            button.frame = 0;
            player.y -= 40;
        } else
            if (confirm("Are you sure you'd like to end?\nClick 'OK' to end, or 'Cancel to return to the game") == true) {
                playState.gameOver();
            } else {
                button.frame = 0;
                player.y -= 40;
